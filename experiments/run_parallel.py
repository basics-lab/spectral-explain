...
def run_job(job_id):
    ...
    command = f'python3 experiments/collect_data.py --seed {job_id} --device {device} --task {args.task} --MAX_B {args.MAX_B} --MAX_ORDER {args.MAX_ORDER} --num_test_samples {args.num_test_samples} --batch_size {args.batch_size} --verbose {args.verbose}'
...
def process_completed_jobs(done, futures, job_ids):
    for future, job_id in [(f, j) for f, j in futures if f in done]:
        try:
            ...
            job_ids.appendleft(job_id)
    # Update the futures list with only the jobs still running
    return [(f, j) for f, j in futures if f not in done]    

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--num_explain", type=int, default=20)
    ...
    seeds = deque([i + args.seed for i in range(args.num_explain)])
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=torch.cuda.device_count()) as executor:
        ...
        while seeds or futures:
            while len(futures) < torch.cuda.device_count():
                job_id = seeds.popleft()
            ...
            futures = process_completed_jobs(done, futures, seeds)
